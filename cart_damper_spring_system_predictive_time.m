%% Compare the influence of different prediction domains on the system
clear; clc; close all;
...
for k=1:L
    ...
    if norm(xjk-x(:,k),2)^2 >= eipilon+1/deta*zeta
        ...
        %% Controller section  
        ut4 = -(Bt'*Qt*Bt+Rt)\Bt'*Qt*(At*xp(:,k)+Ft*f_function(xp(:,k),k)); 
        kexi(k) = ut4(1);                                       % Controller output       
...
%% Set parameters for the model predictive control
Np=10; % Predicted increment size
...
%% Set parameters for the model predictive control
Np=15; % Predicted increment size
...
sum_t3 = 4*L/sum(event3);
sum_J3 = sum(J3);% Calculation of the performance index


%% Set parameters for the model predictive control
Np=20; % Predicted increment size
...
sum_t4 = 5*L/sum(event4);
sum_J4 = sum(J4);% Calculation of the performance index

%% Simulation outcome
subplot(2,1,1)
i=1:L+1;
plot(i,x(1,i),'--r',i,x1(1,i),'--b',i,x2(1,i),'--g',i,x3(1,i),'k',i,x4(1,i),'--c','linewidth',2);
% xlabel('k/step');
ylabel('x_{1,k}');
% title('State output of comparison');
legend('4-step','6-step','10-step','15-step','20-step');
grid on;
axis([0 L -2 3]);
subplot(2,1,2)
i=1:L+1;
plot(i,x(2,i),'--r',i,x1(2,i),'--b',i,x2(2,i),'--g',i,x3(2,i),'k',i,x4(2,i),'--c','linewidth',2);
xlabel('k/step');
ylabel('x_{2,k}');
legend('4-step','6-step','10-step','15-step','20-step');
grid on;
...
subplot(2,1,1)
i=1:L;
plot(i,u(1,i),'--r',i,u1(1,i),'--b',i,u2(1,i),'--g',i,u3(1,i),'k',i,u4(1,i),'--c','linewidth',2);
% xlabel('k/step');
ylabel('u_k');
...
hold on;
scatter(i,event3,sz,'h');
hold on;
scatter(i,event2,sz,'o');
hold on;
...
yticks([1 2 3 4 5])
yticklabels({'4-step','6-step','10-step','15-step','20-step'})
grid on;
axis([0 L 1 5]);
