...
A = [0.1974 0.0102 0.0026 0;-0.2967 0.2847 0.2967 0.0026;0.0001 0 0.1999 0.0103;0.0201 0.0001 -0.0106 0.1999]; 
B = [0;0.2106;0;0];
G = [0;0;0;-0.0033];
Dx = [0.1;0;0;0];
...
xp(:,1) = [-16;-8;-14;10];    % Initial value of the discriminant model
e(:,1) = [0;0;0;0];           % The initial value of the trigger error
kexi(:,1) = 0;                % The initial value of the controller output
xk = zeros(4,tao_x);          % The previous state of the system status
ek = zeros(4,tao_x);          % The previous value of the trigger error
kexik = zeros(1,tao_u);       % The previous value of the controller output
wxk = zeros(4,tao_x);         % The previous value of the disturbance from the sensor to the controller
wuk = zeros(1,tao_u);         % Past disturbance from the controller to the actuator
xjk = zeros(4,1);             % The status output value at the most recent triggering moment
...
eipilon = 0.05;               % Threshold for triggering
...
for k=1:L
    ...
    if norm(xjk-x(:,k),2)^2 >= eipilon+1/deta*zeta
        ...
        %% Controller section  
        ut = -(Bt'*Qt*Bt+Rt)\Bt'*Qt*(At*xp(:,k)+Ft*sin(xp(3,k))); 
        kexi(k) = ut(1);                                       % Controller output       
        xp(:,k+1) = A*xp(:,k)+B*kexi(k)+G*sin(xp(3,k));        % Updating a Prediction Model
        ...
        e(:,k) = xjk-x(:,k);
        kexi(k) = ut(1); 
        xp(:,k+1) = A*xp(:,k)+B*kexi(k)+G*sin(xp(3,k));
        u(:,k) = u(:,k-1);
    end
